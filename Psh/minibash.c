...
int exec(int fd[2], struct cmd *line, int bg) {
	...
	if(pid == 0) {
		...
		close(fd[1]);
		if(!bg) read(fd[1], buffer, sizeof(buffer));
		else {
...
int openF(char * fname) {
	struct stat finf;
	int fd;
	int r = stat(fname, &finf);

	if (r != 0) return open(fname, O_WRONLY|O_CREAT|O_EXCL, 0666);
	return open(fname, O_WRONLY|O_CREAT, 0666);
}
...
int processOne(char cmd[]) {
	...
	{
		if(strcmp(split, ">")==0) skip =1;
		else {
			if(skip!=0) {
				if(skip ==1) outf = split;
			} else {
				args[i++] = split;
			}
